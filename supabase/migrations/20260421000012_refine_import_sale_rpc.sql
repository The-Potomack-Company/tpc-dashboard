-- Follow-up migration: re-apply the full `import_sale_with_departments` RPC
-- body so the WR-06 display_name refinement lands on the live DB.
--
-- Why a new migration: migration 20260421000011_import_sale_rpc.sql was
-- edited in place after it had already been pushed to the remote project.
-- Supabase CLI tracks migrations by version (timestamp), so editing an
-- already-applied migration does not re-apply it on `supabase db push`.
-- A new migration forces the live DB to receive the refined function body.
--
-- CREATE OR REPLACE FUNCTION is idempotent: running this migration multiple
-- times leaves the function in the same final state.
...
begin
  ...
  loop
    ...
    if v_dept_id is null then
      ...
      -- WR-06: refine placeholder display_name on seeded rows
