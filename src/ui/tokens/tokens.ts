 ...
 * runtime source of truth for JS consumers (e.g. Recharts series colors,
 * inline-styled icon glyphs).
 *
 * Mirrors the cataloger Phase 22 layout one-for-one.
 *
 * KNOWN DIVERGENCE (intentional, per cataloger Phase 22 RESEARCH Pitfall 2):
...
// so the strict literal-string types from the light `as const` block do not
// conflict with the genuinely-different dark color values.
