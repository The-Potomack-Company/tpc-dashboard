...
- **Cross-validation:** After all department records for a sale are inserted, re-read them and compare `SUM(dept.*)` to the sale-level totals (hammer, net revenue, lots sold, etc.). Tolerance ±$0.01 per `numeric(14,2)` rounding. Mismatch → log warning AND set `sales.validation_warning = true` (requires new column added via new migration).
